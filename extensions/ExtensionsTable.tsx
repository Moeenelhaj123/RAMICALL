...
export function ExtensionsTable({ 
  ...
  const columns: TableColumn<ExtensionRow>[] = [
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          <Avatar className="h-8 w-8 flex-shrink-0">
            <AvatarImage src={undefined} alt={record.callerIdName || record.extension} />
            <AvatarFallback className="bg-slate-100 text-slate-600 font-medium text-xs">
              {record.callerIdName ? record.callerIdName.split(' ').map(n => n[0]).join('') : record.extension.slice(-2)}
            </AvatarFallback>
          </Avatar>
          <div className="min-w-0 flex-1">
            <div className="font-semibold text-slate-900 truncate">{record.callerIdName || '—'}</div>
            <div className="text-sm text-slate-500 truncate">{record.email || '—'}</div>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (value) => <div className="font-semibold text-slate-900">{value}</div>,
    },
    {
      key: 'device',
      title: 'Device Status',
